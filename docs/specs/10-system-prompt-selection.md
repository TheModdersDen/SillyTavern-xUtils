...
Allow users to choose which system prompt xUtils sends to the LLM when building tracker data. Currently the system prompt is implicitly dictated by the SillyTavern connection profile (`profile.sysprompt`). This spec adds two explicit modes:
...
xUtils ships a recommended system prompt preset that gets installed into SillyTavern's system prompt library. Users select it via Mode B. This avoids a redundant custom-text editor inside xUtils — the prompt lives where SillyTavern already manages system prompts.
...
- **Mode B** lets users pick any saved system prompt — including the xUtils-optimized one we ship — without creating a dedicated connection profile.

A dedicated Mode C (custom textarea in xUtils settings) was considered and rejected: it duplicates functionality SillyTavern already provides and adds UI surface that needs to be maintained. Users who want a custom prompt can save it in SillyTavern and select it via Mode B.
...
5. xUtils then injects previous tracker snapshots (`includeXUtilsMessages`), optional World Info allowlist entries, and finally appends the tracker-specific user prompt (`settings.prompt` / JSON / XML template).

### What this means

- The system prompt is **always** the one named in the connection profile.
- There is **no UI** in xUtils settings to override or inspect which system prompt is active.
- Changing the system prompt requires editing the connection profile in SillyTavern core settings.

## Goals

- Let the user choose between Mode A / B via a dropdown in xUtils settings.
- For Mode B, provide a sub-selector that lists all saved SillyTavern system prompts.
- Ship a recommended xUtils system prompt as a regular SillyTavern system prompt preset.
...
- A custom system-prompt text editor inside xUtils settings (use SillyTavern's built-in editor instead).
...
### Shipped system prompt preset: "xUtils"

xUtils installs a SillyTavern system prompt preset named **"xUtils"** on first load (if no preset with that name exists). This prompt is designed to complement the tracker prompt (`settings.prompt` / JSON / XML template) which is appended later as a user message. The two prompts have distinct responsibilities:

| Concern | System prompt ("xUtils" preset) | Tracker prompt (user message, appended last) |
|---------|-----------------------------------|----------------------------------------------|
...
```ts
/**
 * Controls where xUtils gets its system prompt for tracker generation.
...
1. Check if a SillyTavern system prompt named `"xUtils"` already exists via `getPresetManager("sysprompt")`.
2. If not, create it with the shipped text above.
3. Do **not** overwrite if the user has edited it.

This is implemented via `ensureXUtilsSystemPromptPresetInstalled()`, called during startup before rendering settings. It uses SillyTavern's runtime preset manager `savePreset(name, data)` API and only creates the preset when it is missing.
...
│  manager. The "xUtils" preset is optimized for    │
│  tracker generation.                                │
└─────────────────────────────────────────────────────┘
```
...
- A hint below the dropdown tells users about the shipped "xUtils" preset and where to edit prompts.
...
| File | Change |
|------|--------|
| `src/config.ts` | Add `trackerSystemPromptMode`, `trackerSystemPromptSavedName` to `ExtensionSettings`. Add `XUTILS_SYSTEM_PROMPT_PRESET_NAME` and `XUTILS_SYSTEM_PROMPT_TEXT` constants. Add defaults to `defaultSettings`. |
...
| `src/index.tsx` | Ensures the shipped `xUtils` system prompt preset exists before rendering settings. |
...
3. **`prefer_character_prompt` interaction**: resolved in implementation. When Mode B is active, xUtils injects the selected saved prompt as a dedicated leading `system` message instead of temporarily mutating SillyTavern's global `prefer_character_prompt` setting.

## Acceptance criteria

- [x] Mode dropdown (A/B) appears in xUtils settings.
...
- [x] A "xUtils" system prompt preset is installed into SillyTavern on first load.
